...
export default async function HourBanksPage() {
  ...
  const [{ data: banks }, { data: customers }, { data: settings }] = await Promise.all([
    supabase.from("hour_banks_summary").select("*").order("created_at", { ascending: false }),
  ...
  const items: HourBankListItem[] = (banks ?? []).map((b) => ({
    ...
    status: (b.status ?? "active") as "active" | "depleted" | "expired" | "cancelled",
