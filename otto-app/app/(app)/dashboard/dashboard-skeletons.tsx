      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-cream-paper shadow-card rounded-2xl p-5">
            <div className="bg-ink-line mb-3 h-8 w-14 animate-pulse rounded" />
      ...
      <div className="flex items-center gap-6 px-1">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="flex items-baseline gap-2">
            <div className="bg-ink-line h-5 w-6 animate-pulse rounded" />
            <div className="bg-ink-line h-3 w-20 animate-pulse rounded" />
          </div>
        ))}
      </div>
      ...
      <ul className="divide-ink-line/60 divide-y">
        {Array.from({ length: 5 }).map((_, i) => (
          <li key={i} className="flex items-start gap-3 py-2.5">
            <div className="bg-ink-line mt-1 h-4 w-4 shrink-0 animate-pulse rounded" />
